...
?>
<div class="content group listings-container">
...
?>
	...
	<ul class="leadingblock <?php echo $classnum; ?> group row listing-row">
		<?php
		...
		for ($i=0; $i<$leadnum; $i++) :
			$item = $items[$i];
			$fc_item_classes = 'listings-item';
			if ($doing_cat_order)
     		$fc_item_classes .= ($i==0 || ($items[$i-1]->rel_catid != $items[$i]->rel_catid) ? ' fc_cat_item_1st' : '');
			$fc_item_classes .= $i%2 ? ' item_even' : ' item_on';
			$fc_item_classes .= ' item_col'.($i%$lead_cols + 1);
		...
		<li id="listing_item_<?php echo $i; ?>" class="<?php echo $fc_item_classes; ?>" <?php echo $microdata_itemtype_code; ?> style="overflow: hidden;">
...
?>
	...
	<ul class="listings <?php echo $classnum; ?> group">
		<?php
		...
		for ($i=$leadnum; $i<$count; $i++) :
			...
			if ($doing_cat_order)
     		$fc_item_classes .= ($i==0 || ($items[$i-1]->rel_catid != $items[$i]->rel_catid) ? ' fc_cat_item_1st' : '');
			$fc_item_classes .= ($i-$leadnum)%2 ? ' item_even' : ' item_on';
			$fc_item_classes .= ' item_col'.($i%$intro_cols + 1);
